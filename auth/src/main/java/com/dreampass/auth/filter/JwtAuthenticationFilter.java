...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            try {
                ...
                writeJsonError(response, HttpServletResponse.SC_UNAUTHORIZED, "Token无效，请重新登录");
                return;
            }
        } else {
            writeJsonError(response, HttpServletResponse.SC_UNAUTHORIZED, "Token无效，请重新登录");
    ...
    private void writeJsonError(HttpServletResponse response, int code, String message) throws IOException {
        response.setStatus(HttpServletResponse.SC_OK); // 保持 HTTP 状态码 200
        response.setContentType("application/json;charset=UTF-8");
        String json = new ObjectMapper().writeValueAsString(Result.failure(code, message));
